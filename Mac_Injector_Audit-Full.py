...
class MacInjector:
    ...
    def get_executable_info(self, app_path):
        ...
        try:
            ...
            # keep behavior non-fatal; print for debug
...
"""
            ...
            # PoC visual: opens Calculator.app
            code = """
#include <stdio.h>
#include <unistd.h>
...
"""
    ...
    def run_single_method(self, method_name, app_path):
        """Run a single method by name and return results dict in same format as test_all_methods"""
        method_map = {
            "DYLD_INSERT_LIBRARIES": self.method_dyld_insert_libraries,
            "DYLD_INSERT_WITH_FLAGS": self.method_dyld_insert_with_flags,
            "WEAK_DYLIB_HIJACKING": self.method_weak_dylib_hijacking,
        ...
        results = {}
        # If method is an insert type, compile stealth dylib (visual PoC)
        if method_name in ("DYLD_INSERT_LIBRARIES", "DYLD_INSERT_WITH_FLAGS"):
            dylib_path = self.compile_dylib("stealth")
            func = method_map.get(method_name)
            if func and dylib_path:
                success, message = func(app_path, dylib_path)
            elif func:
                success, message = False, "Failed to compile dylib"
            else:
                success, message = False, "Unknown method"
            results[method_name] = {'success': success, 'message': message, 'method': method_name}
            return results

        # Non-insert methods
        func = method_map.get(method_name)
        if func:
            success, message = func(app_path)
            results[method_name] = {'success': success, 'message': message, 'method': method_name}
        else:
            results[method_name] = {'success': False, 'message': 'Unknown method', 'method': method_name}
    ...
    def interactive_mode(self):
        """Interactive mode for user selection with submenu for method selection"""
        applications = self.get_applications()
        ...
        while True:
            ...
            try:
                ...
                if 0 <= choice < len(applications):
                    ...
                    # NEW: let user choose which injection technique to run
                    method_choice = self.select_injection_method()
                    
                    if method_choice == "ALL_METHODS":
                        results = self.test_all_methods(selected_app, "stealth")
                    else:
                        results = self.run_single_method(method_choice, selected_app)
                    
                    # Use the same report printer
    ...
    def automatic_scan(self):
        ...
        for app_path in applications[:10]:
            ...
            # Each above returns (bool, message); access first element
...
def main():
    ...
    parser.add_argument('-m', '--method', help='Method name to test (optional). One of: DYLD_INSERT_LIBRARIES, DYLD_INSERT_WITH_FLAGS, WEAK_DYLIB_HIJACKING, DYLD_HIJACKING, FRIDA_INJECTION, BINARY_PATCHING, ALL_METHODS')
    ...
    try:
        if args.interactive:
            ...
            if os.path.exists(args.target):
                # If a method specified, run single; otherwise run all
                if args.method:
                    results = injector.run_single_method(args.method, args.target)
                else:
